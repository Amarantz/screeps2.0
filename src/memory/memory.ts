...
import SETTINGS, { USE_SCREEPS_PROFILER, PROFILER_COLONY_LIMIT } from "../settings";
...
export class Mem {
  ...
  static shouldRun(): boolean {
    ...
    if (USE_SCREEPS_PROFILER && Game.time % 10 == 0) {
      log.warning(`Profiling is currently enabled; only ${PROFILER_COLONY_LIMIT} brains will be run!`);
    }
  ...
  private static _setDeep(object: any, keys: string[], value: any): void {
	 ...
	 * Ex: Mem.setDeep(Memory.brains, 'E1S1.miningSites.siteID.stats.uptime', 0.5)
	 */
  ...
  private static cleanBrains() {
    // Clear memory of dead brains
    for (const name in Memory.brains) {
      const room = Game.rooms[name];
      if (!(room && room.my)) {
        // Delete only if "persistent" is not set - use case: praise rooms
        if (!Memory.brains[name].persistent) {
          delete Memory.brains[name];
          delete global[name];
        }
      }
    }

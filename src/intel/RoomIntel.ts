import {Cartographer, ROOMTYPE_CORE} from '../utils/Cartographer';
import {
	packCoord,
	packCoordList,
	packId,
...
import {ema, getCacheExpiration} from '../utils/utils';
import { profile } from '../profiler';
import { ExpansionEvaluator } from 'strategy/ExpansionEvaluation';
...
export class RoomIntel {
	...
	static getMyZoneStatus(): 'normal' | 'novice' | 'respawn' {
		const oneOfMyColonies = _.first(_.values(BigBrain.brains)) as any;
	...
	static run(): void {
		...
		for (const roomName in Game.rooms) {
			...
			if (BigBrain.brainsMaps[room.name]) { // if it is an owned or outpost room

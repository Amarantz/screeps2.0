...
import { toCreep, Bot } from "bot/Bot";
import { BOOST_TIERS } from "resources/map_resoures";
import { isStructure, isOwnedStructure, isStandardBot, isCreep } from "declarations/typeGuards";
...
export class CombatIntel {
	...
	private findBestExit(matrix: CostMatrix, towers: StructureTower[],
		...
		const ret = Pathing.findPath(this.brain.pos, destination.pos, {range: 1});
		if (!ret.incomplete) {
			bestExit = _.find(ret.path, p => p.roomName == this.room!.name);
		}

		// Figure out possible exits to go from enemy room back to brain in a reasonable amount of time
		...
		if (!bestExit) {
			...
			for (const direction in exitData) {
				...
				const allowedRooms = Pathing.findRoute(this.brain.name, roomName);
	...
	findSimpleSiegeFallback(): RoomPosition {
		const ret = Pathing.findPath(this.brain.pos, this.directive.pos, {range: 23});
		if (ret.incomplete) {
	...
	// 	// let ret = Pathing.findSwarmPath(this.brain.pos, this.directive.pos, clearance.width, clearance.height,
	// 	// 								{ignoreCreeps: true});
	// 	const ret = Pathing.findPath(this.brain.pos, this.directive.pos);
	// 	const path = ret.path.reverse();
	// 	const acceptablePositions = _.filter(path, pos => pos.roomName == this.brain.name && pos.rangeToEdge > 1);
	// 	const swarmSize = Math.max(clearance.width, clearance.height);
	// 	const posIndex = (swarmSize + 1) * swarmIndex;
					...
					if (!boost && countIntendedBoosts && creep.my) {
						if (creep.memory.needBoosts) {
							boost = _.find(creep.memory.needBoosts,
										   boost => boost == BOOST_TIERS.heal.T1 ||
	 ...
	 */
	static isHealer(bot: Creep | Bot): boolean {
		const creep = toCreep(bot);
		const healParts = _.filter(bot.body, part => part.type == HEAL).length;
		const attackParts = _.filter(bot.body, part => part.type == ATTACK).length;
		const rangedAttackParts = _.filter(bot.body, part => part.type == RANGED_ATTACK).length;
	 ...
	 */
	static getAttackPotential(creep: Creep, countIntendedBoosts = false): number {
		return this.cache(creep, 'attackPotential', () => _.sum(creep.body, function(part) {
			...
			if (part.type == ATTACK) {
				...
				if (!boost && countIntendedBoosts && creep.my) {
					if (creep.memory.needBoosts) {
						boost = _.find(creep.memory.needBoosts,
									   boost => boost == BOOST_TIERS.attack.T1 ||
	 ...
	 */
	static getRangedAttackPotential(creep: Creep, countIntendedBoosts = false): number {
			...
			_.sum(creep.body, function(part) {
				...
				if (part.type == RANGED_ATTACK) {
					...
					if (!boost && countIntendedBoosts && creep.my) {
						if (creep.memory.needBoosts) {
							boost = _.find(creep.memory.needBoosts,
										   boost => boost == BOOST_TIERS.ranged.T1 ||
	 ...
	 * Attack potential of a single creep in units of effective number of parts
	 */
	static getDismantlePotential(creep: Creep, countIntendedBoosts = false): number {
		return this.cache(creep, 'dismantlePotential', () => _.sum(creep.body, function(part) {
			...
			if (part.type == WORK) {
				...
				if (!boost && countIntendedBoosts && creep.my) {
					if (creep.memory.needBoosts) {
						boost = _.find(creep.memory.needBoosts,
									   boost => boost == BOOST_TIERS.dismantle.T1 ||
	...
	static getDismantleDamage(creep: Creep | Bot): number {
		return DISMANTLE_POWER * this.getDismantlePotential(toCreep(creep));
	}

	static getRepairPotential(creep: Creep, countIntendedBoosts = false): number {
		return this.cache(creep, 'repairPotential', () => _.sum(creep.body, function(part) {
			...
			if (part.type == WORK) {
				...
				if (!boost && countIntendedBoosts && creep.my) {
					if (creep.memory.needBoosts) {
						boost = _.find(creep.memory.needBoosts,
									   boost => boost == BOOST_TIERS.construct.T1 ||
	...
	static getRepairPower(creep: Creep | Bot): number {
	...
	static getMassAttackDamageTo(attacker: Creep | Bot, target: AnyCreep | Structure): number {
		...
		return rangedMassAttackPower * this.getRangedAttackPotential(isStandardBot(attacker) ? attacker.creep : attacker);
	}

	/**
	 * Total damage to enemy creeps done by attacker.rangedMassAttack()
	 */
	static getMassAttackDamage(attacker: Creep | Bot, targets = attacker.room.hostiles, checkRampart = true): number {
	 ...
	 */
	static rating(creep: Creep | Bot): number {
	 ...
	 * Total attack/rangedAttack/heal potentials for a group of creeps
	 */
	static getMyCombatPotentials(bots: Bot[], countIntendedBoosts = true): CombatPotentials {
		const attack = _.sum(bots, bot => this.getAttackPotential(bot.creep, countIntendedBoosts));
		const rangedAttack = _.sum(bots, bot => this.getRangedAttackPotential(bot.creep, countIntendedBoosts));
		const heal = _.sum(bots, bot => this.getHealPotential(bot.creep, countIntendedBoosts));
		const dismantle = _.sum(bots, bot => this.getDismantlePotential(bot.creep, countIntendedBoosts));
		return {attack, ranged: rangedAttack, heal, dismantle};
	}
	 ...
	 */
	static predictedDamageAmount(attacker: Creep | Bot, target: AnyCreep, attackType: 'attack' | 'rangedAttack',

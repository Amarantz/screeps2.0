...
import { Reaction, Abathur } from "resources/abathur";
import { log } from "console/log";
import { REAGENTS, BOOST_PARTS, BoostType, BOOST_TIERS } from "resources/map_resoures";
import { rightArrow, ema, randint } from "utils/utils";
...
interface EngineeringBayMemory {
	status: number;
	statusTick: number;
...
const getDefaultEngineeringBayMemory: () => EngineeringBayMemory = () => ({
...
export class EngineeringBay extends Component {

	terminal: StructureTerminal;							// The brain terminal
	terminalNetwork: TerminalNetworkV2;						// Reference to Overmind.terminalNetwork
	labs: StructureLab[];									// Brain labs
	...
	constructor(brain: Brain, terminal: StructureTerminal) {
		super(brain, terminal, 'engineeringBay');
		this.memory = Mem.wrap(this.brain.memory, 'engineeringBay', getDefaultEngineeringBayMemory);
		// Register physical components
		this.terminal = terminal;
		this.terminalNetwork = BigBrain.terminalNetwork as TerminalNetworkV2;
		this.labs = brain.labs;
		// Reserve some easily-accessible labs which are restricted not to be reagent labs
		const restrictedLabs = this.brain.bunker
							   ? _.filter(this.labs, lab => lab.pos.findInRange(this.brain.spawns, 1).length > 0)
		...
		const getReagentLabs: () => StructureLab[] = () => {
			...
			// 	const reagentLabPositions = _.map(reagentLabSpots, coord => getPosFromBunkerCoord(coord, this.brain));
		...
		// this.boostQueue = {};
		this.neededBoosts = {};
		if (this.brain.commandCenter && this.brain.layout == 'twoPart') { // TODO: deprecate soon
			// in two-part layout, evolution chamber shares a common request group with command center
			this.transportRequests = this.brain.commandCenter.transportRequests;
		} else {
			// otherwise (in bunker layout), it uses brain/hatchery transport requests
			this.transportRequests = this.brain.transportRequest;
		}
	}
		...
		switch (this.memory.status) {
			case LabStatus.Idle:
				if (this.memory.activeReaction) {
					log.info(`${this.brain.print}: starting synthesis of ${reagents[0]} + ${reagents[1]} ` +
			...
			case LabStatus.AcquiringMinerals: // "We acquire more mineralzzz"
				if (_.all(reagents, reagent => this.brain.assets[reagent] >= amount)) {
	...
	bestBoostAvailable(boostType: BoostType, amount: number): ResourceConstant | undefined {
		if (PHASE != 'run') {
			log.error(`EngineeringBay.bestBoostAvailable() must be called in the run() phase!`);
		}
		...
		for (const boost of [boosts.T3, boosts.T2, boosts.T1]) {
			if (this.brain.assets[boost] >= amount) {
				return boost;
			} else if (this.terminalNetwork.canObtainResource(this.brain, boost, amount)) {
				return boost;
			}
		}
	...
	private lockLabFromTerminalNetwork(lab: StructureLab) {
		if (lab.mineralType && lab.store[lab.mineralType]) {
			this.terminalNetwork.lockResource(this.brain, lab.mineralType, lab.store[lab.mineralType]);
		}
	}
	...
	init(): void { // This gets called after every Overlord.init() so you should have all your boost requests in already
		...
		for (const boost in this.neededBoosts) {
			...
			if (amountNeeded > this.brain.assets[boost]) {
				this.debug(`Requesting boost from terminal network: ${this.neededBoosts[boost]} ${boost}`);
				this.terminalNetwork.requestResource(this.brain, <ResourceConstant>boost, amountNeeded);
			} else {
				this.debug(`Locking boost from terminal network: ${this.neededBoosts[boost]} ${boost}`);
				this.terminalNetwork.lockResource(this.brain, <ResourceConstant>boost, amountNeeded);
			}
		}
			...
			switch (this.memory.status) {
				...
				case LabStatus.AcquiringMinerals:
					_.forEach(reagents, reagent => {
						if (this.brain.assets[reagent] < amount) {
							this.terminalNetwork.requestResource(this.brain, reagent, amount);
						} else {
							this.terminalNetwork.lockResource(this.brain, reagent, amount);
						}
					});
					break;
				case LabStatus.LoadingLabs:
					_.forEach(reagents, reagent => this.terminalNetwork.lockResource(this.brain, reagent, amount));
					break;
				case LabStatus.Synthesizing:
					_.forEach(this.reagentLabs, lab => {
	...
	run(): void {
		...
		if (!this.memory.activeReaction && !this.memory.suspendReactionsUntil) {
			const nextReaction = Abathur.getNextReaction(this.brain);
			...
			// the brain request states have already been registered in the init() phase. In any case, this adds an
			...
			if (nextReaction) {
				...
				const sleepTime = EngineeringBay.settings.sleepTime + randint(0, 20);
	...
	private drawLabReport(coord: Coord): Coord {
		let {x, y} = coord;
		const height = 2;
		const titleCoords = Visualizer.section(`${this.brain.name} Evolution Chamber`,
	...
	private stats(): void {
		// Stats.log(`colonies.${this.brain.name}.engineeringBay.totalProduction`, this.memory.stats.totalProduction);
		...
		Stats.log(`colonies.${this.brain.name}.engineeringBay.avgUsage`, this.memory.stats.avgUsage);
	}

import { AnyBot, normalizeAnyBot } from "bot/AnyBot";
import { sameCoord, isExit, normalizePos, getTerrainCosts } from "./helpers";
import { Pathing, PathOptions } from "./Pathing";
import { Bot } from "bot/Bot";
import { MatrixLib, MatrixOptions } from "matrix/MatrixLib";
import { isPowerBot, isStandardBot, isAnyBot } from "declarations/typeGuards";
import { profile } from "profiler";
import { Roles } from "creepSetup/setup";
import { log } from "../console/log";
import { rightArrow, minBy } from "utils/utils";
import { insideBunkerBounds } from "roomPlanner/layout/bunker";
...
/**
 * This is the movement library for BigBrain. It was originally based on BonzAI's Traveler library, but it has been
 * extensively modified to integrate more tightly with the BigBrain framework and add additional functionality.
 */
@profile
export class Movement {
	 ...
	 */
	static goTo(creep: AnyBot, destination: HasPos | RoomPosition, opts: MoveOptions = {}): number {

		if (creep.blockMovement && !opts.force) {
			return ERR_BUSY;
		}
		if (isStandardBot(creep)) {
			if (creep.spawning) {
				return NO_ACTION;
			}
			if (creep.fatigue > 0) {
		...
		if (!moveData.path || moveData.path.length == 0) {
			newPath = true;
			if (isStandardBot(creep) && creep.spawning) {
				return ERR_BUSY;
			}
			state.destination = destination;
			...
			if (isPowerBot(creep)) {
				if (pathOpts.terrainCosts) {
					log.error(`${creep.print}: MoveOptions.terrainCosts not supported by PowerBot`);
				}
				pathOpts.terrainCosts = {plainCost: 1, swampCost: 1};
			} else if (isStandardBot(creep)) {
				if (!pathOpts.terrainCosts) {
					pathOpts.terrainCosts = getTerrainCosts(creep.creep);
				}
			} else {
				log.error(`${creep.print} is not Bot or PowerBot!`);
			}
	...
	//  */
	// private static traversePortalWaypoint(creep: AnyBot, portalPos: RoomPosition): boolean {
	...
	//  */
	// private static crossPortal(creep: AnyBot, portalPos: RoomPosition): boolean {
	...
	private static getPushPriority(creep: AnyCreep | AnyBot): number {
	...
	private static shouldPush(pusher: AnyCreep | AnyBot, pushee: AnyCreep | AnyBot): boolean {
		if (this.getPushPriority(pusher) < this.getPushPriority(pushee)) {
			...
			pushee = normalizeAnyBot(pushee);
			if (isAnyBot(pushee)) {
				if (isStandardBot(pushee)) {
	...
	private static getPushDirection(pusher: AnyBot | AnyCreep, pushee: AnyBot | AnyCreep): DirectionConstant {
	...
	private static findBlockingCreep(creep: AnyBot): AnyCreep | undefined {
	...
	static pushCreep(creep: AnyBot, otherCreep: AnyCreep | AnyBot): boolean {
		if (!otherCreep.memory) return false;
		otherCreep = normalizeAnyBot(otherCreep);
		...
		if (isStandardBot(otherCreep)) {
			...
			log.warning(`${otherCreep.name}@${otherCreep.pos.print} is not Bot! (Why?)`);
			if (outcome == OK) {
	...
	static vacatePos(pos: RoomPosition, suicide = false): boolean {
		...
		const nearbyCreeps = _.compact(_.map(pos.findInRange(FIND_MY_CREEPS, 2),
											 creep => BigBrain.bots[creep.name])) as Bot[];
		...
		const blockingCreep = BigBrain.bots[creep.name];
	 ...
	 */
	static recursivePush(creep: Bot, excludePos: RoomPosition[] = []): RoomPosition | undefined {
		...
		if (movePos) {
			...
			for (const otherPos of availablePositions) {
				...
				const otherBot = BigBrain.bots[otherCreep.name];
				if (!otherBot) continue;
				movePos = this.recursivePush(otherBot, excludePos.concat(creepPos));
				if (movePos) {
	 ...
	 * Travel to a room
	 */
	static goToRoom(creep: AnyBot, roomName: string, options: MoveOptions = {}): number {
		options.range = 20;
	 ...
	 */
	static park(creep: AnyBot, pos: RoomPosition = creep.pos, maintainDistance = false): number {
		const road = creep.pos.lookForStructure(STRUCTURE_ROAD);
		if (!road) return OK;

		// Move out of the bunker if you're in it
		if (!maintainDistance && creep.brain && creep.brain.bunker && insideBunkerBounds(creep.pos, creep.brain)) {
			return this.goTo(creep, creep.brain.controller.pos);
		}
	 ...
	 */
	static moveOffCurrentPos(creep: AnyBot): number | undefined {
	 ...
	 */
	static moveOnExit(creep: AnyBot): ScreepsReturnCode | undefined {
		if (creep.pos.rangeToEdge > 0) {
			if (isStandardBot(creep) && creep.fatigue > 0) {
				return;
			}
	 ...
	 */
	static moveOffExit(creep: AnyBot, towardPos?: RoomPosition, avoidSwamp = true): ScreepsReturnCode | NO_ACTION {
		if (!creep.pos.isEdge) {
			return NO_ACTION;
		}
	 ...
	 */
	static pairwiseMove(leader: AnyBot, follower: AnyBot, target: HasPos | RoomPosition,
		...
		} else if (isStandardBot(follower) && follower.fatigue == 0) {
	...
	static combatMove(creep: Bot, approach: PathFinderGoal[], avoid: PathFinderGoal[],
	 ...
	 */
	static invasionMove(creep: Bot, destination: RoomPosition | HasPos, opts: MoveOptions = {}): number {
	 ...
	 */
	static kite(creep: AnyBot, avoidGoals: (RoomPosition | HasPos)[], options: MoveOptions = {}): number | undefined {
		_.defaults(options, {
			fleeRange   : 5,
			terrainCosts: isPowerBot(creep) ? {plainCost: 1, swampCost: 1} : getTerrainCosts((<Creep>creep.creep)),
		});
	 ...
	 */
	static flee(creep: AnyBot, avoidGoals: (RoomPosition | HasPos)[],
				dropEnergy = false, opts: MoveOptions = {}): number | undefined {

		if (avoidGoals.length == 0) {
			return; // nothing to flee from
		}
		const terrainCosts = isPowerBot(creep) ? {plainCost: 1, swampCost: 1} : getTerrainCosts((<Creep>creep.creep));
	...
	private static serializeState(creep: AnyBot, destination: RoomPosition, state: MoveState, moveData: MoveData,
	...
	private static isStuck(creep: AnyBot, state: MoveState): boolean {
		let stuck = false;
		if (state.lastCoord !== undefined) {

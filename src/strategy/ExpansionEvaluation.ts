...
import { Cartographer, ROOMTYPE_CONTROLLER, ROOMTYPE_SOURCEKEEPER, ROOMTYPE_ALLEY, ROOMTYPE_CORE, ROOMTYPE_CROSSROAD } from "utils/Cartographer";
...
export class ExpansionEvaluator {
	...
	static computeTheoreticalMiningEfficiency(dropoffLocation: RoomPosition, room: string, verbose = false)
		...
		for (const source of sourcePositions) {
			const setup = roomType == ROOMTYPE_CONTROLLER ? Setups.worker.miner.standard.generateMaxedBody()
				: Setups.worker.miner.sourceKeeper.generateMaxedBody();
		...
		if (roomType == ROOMTYPE_CONTROLLER) {
			...
			if (!controller) {
				...
				const setup = Setups.claimer.reserve.generateMaxedBody();
		...
		for (const source of sourcePositions) {
			const setup = Setups.transport.default.generateMaxedBody();

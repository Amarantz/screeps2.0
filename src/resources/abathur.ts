...
export class Abathur {
	...
	/**
	 * Compute the next reaction that a brain should undertake based on local and global stockpiles of all target
	 * compounds.
	 */
	static getNextReaction(brain: Brain): Reaction | undefined {
		const BATCH_SIZE = Abathur.settings.batchSize;
		const globalAssets = BigBrain.terminalNetwork.getAssets();
		const numColonies = _.filter(getAllBrains(), brain => !!brain.terminal).length;

		let possibleReactions = REACTION_PRIORITIES;
		if (brain.labs.length < 10) { // don't make the really long cooldown stuff if you don't have all labs
		...
		const maxAmountOfEachBoostPerBrain = 50000;
		const maxBatches = Math.ceil(maxAmountOfEachBoostPerBrain / BATCH_SIZE);
		for (const batchNum of _.range(1, maxBatches)) {
			...
			nextTargetResource = _.find(possibleReactions, resource => {
				...
				if (resource == RESOURCE_GHODIUM || Abathur.isIntermediateReactant(resource)) {
					// If the brain already has more of this this intermediate than it wants, skip it
					if (brain.assets[resource] > BigBrain.terminalNetwork.thresholds(brain, resource).target) {
						return false;
					}
				}
				...
				const localShortage = brain.assets[resource] < batchNum * BATCH_SIZE;
				if (globalShortage || localShortage) {
					...
					const reagent1Available = brain.assets[reagent1] >= BATCH_SIZE ||
											  BigBrain.terminalNetwork.canObtainResource(brain, reagent1, BATCH_SIZE);
					const reagent2Available = brain.assets[reagent2] >= BATCH_SIZE ||
											  BigBrain.terminalNetwork.canObtainResource(brain, reagent2, BATCH_SIZE);
	...
	private static canReceiveBasicMineralsForReaction(mineralQuantities: { [resourceType: string]: number },
		...
		for (const mineral in mineralQuantities) {
			if (!Abathur.someBrainHasExcess(<ResourceConstant>mineral, mineralQuantities[mineral])) {
				return false;
			}
		}
		return true;
	...
	private static canBuyBasicMineralsForReaction(mineralQuantities: { [resourceType: string]: number }): boolean {
		...
		for (const mineral in mineralQuantities) {
			...
			if (BigBrain.tradeNetwork.priceOf(<ResourceConstant>mineral) > maxPrice) {
				return false;
			}
		}
		return true;
	...
	private static hasExcess(brain: Brain, mineralType: ResourceConstant, excessAmount = 0): boolean {
		return brain.assets[mineralType] - excessAmount > Abathur.stockAmount(mineralType);
	}

	private static someBrainHasExcess(mineralType: ResourceConstant, excessAmount = 0): boolean {
		return _.any(getAllBrains(), brain => Abathur.hasExcess(brain, mineralType, excessAmount));
	}

	/**
	 * Generate a queue of reactions to produce the most needed compound
	 */
	private static getReactionQueue(brain: Brain, verbose = false): Reaction[] {
		...
		for (const stocks of stocksToCheck) {
			for (const resourceType in stocks) {
				const amountOwned = brain.assets[resourceType];
				const amountNeeded = stocks[resourceType];
				if (amountOwned < amountNeeded) { // if there is a shortage of this resource
					const reactionQueue = Abathur.buildReactionQueue(brain, <ResourceConstant>resourceType,
																	 amountNeeded - amountOwned, verbose);

					const missingBaseMinerals = Abathur.getMissingBasicMinerals(brain, reactionQueue);
	 ...
	 */
	private static buildReactionQueue(brain: Brain, mineral: ResourceConstant, amount: number,
		...
		if (verbose) console.log(`Abathur@${brain.room.print}: building reaction queue for ${amount} ${mineral}`);
		...
		for (const ingredient of Abathur.enumerateReactionProducts(mineral)) {
			...
			if (ingredient != mineral) {
				if (verbose) {
					console.log(`productionAmount: ${productionAmount}, assets: ${brain.assets[ingredient]}`);
				}
				productionAmount = Math.max(productionAmount - (brain.assets[ingredient]), 0);
			}
	 ...
	 */
	private static getMissingBasicMinerals(brain: Brain, reactionQueue: Reaction[],
		...
		if (verbose) console.log(`assets: ${JSON.stringify(brain.assets)}`);
		const missingBasicMinerals: { [resourceType: string]: number } = {};
		for (const mineralType in requiredBasicMinerals) {
			const amountMissing = requiredBasicMinerals[mineralType] - brain.assets[mineralType];
			if (amountMissing > 0) {

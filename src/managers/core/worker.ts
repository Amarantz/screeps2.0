...
import { Brain, brainStage, DEFCON } from "Brian";
...
export class WorkerManager extends Manager {
	...
	constructor(colony: Brain, priority = ManagerPriority.ownedRoom.work) {
		...
		// 		s.hits < WorkerManager.settings.barrierHits[this.brain.level]
		// 		&& this.brain.roomPlanner.barrierPlanner.barrierShouldBeHere(s.pos)
		// 	), s => s.hits), 25);
		// this.criticalBarriers = $.structures(this, 'criticalBarriers', () =>
		// 	_.filter(this.fortifyBarriers,
		// 			 barrier => barrier.hits < WorkerManager.settings.barrierHits.critical), 10);
		...
		this.repairStructures = $.structures(this, 'repairStructures', () =>
			_.filter(this.brain.repairables, structure => {
				if (structure.structureType == STRUCTURE_CONTAINER) {
					// only repair containers in owned room
					if (structure.pos.roomName == this.brain.name) {
		...
		const homeRoomName = this.brain.room.name;
		const defcon = this.brain.defcon;
		// Filter constructionSites to only build valid ones
		const room = this.brain.room as any;
		const level = this.brain.controller.level;
		this.constructionSites = _.filter(this.brain.constructionSites, function(site) {
		...
		if (this.room.find(FIND_NUKES).length > 0) {
			for (const rampart of this.brain.room.ramparts) {
		...
		const totalFortifyHitsRemaining = _.sum(this.fortifyBarriers, barrier =>
			Math.min(WorkerManager.settings.barrierHits[this.brain.level] - barrier.hits, 0));
		...
		// Register workers
		this.workers = this.bots(Roles.worker);
	}

	private neededRampartHits(rampart: StructureRampart): number {
		let neededHits = WorkerManager.settings.barrierHits[this.brain.level];
	...
	init() {
		let setup = this.brain.level == 1 ? Setups.worker.builders.early : Setups.worker.builders.default;
		const workPartsPerWorker = setup.getBodyPotential(WORK, this.brain);
		let numWorkers: number;
		if (this.brain.stage == brainStage.Infant) {
			numWorkers = $.number(this, 'numWorkers', () => {
				...
				const energyMinedPerTick = _.sum(_.map(this.brain.miningSites, function(site) {
			...
			if (this.brain.roomPlanner && this.brain.roomPlanner.memory && this.brain.roomPlanner.memory.relocating) {
				...
				numWorkers = $.number(this, 'numWorkers', () => {
					...
					const activeRooms = _.filter(this.brain.roomNames, roomName => this.brain.isRoomActive(roomName));
					const paveTicks = _.sum(activeRooms,
											roomName => this.brain.roadLogistics.energyToRepave(roomName));
					let fortifyTicks = 0;
					const shouldFortify = this.brain.assets.energy > WorkerManager.settings.fortifyDutyThreshold;
					if (shouldFortify) {
						fortifyTicks = 0.25 * _.sum(this.fortifyBarriers, barrier =>
							Math.max(0, WorkerManager.settings.barrierHits[this.brain.level]
					...
					if (this.brain.controller.ticksToDowngrade <= (this.brain.level >= 4 ? 10000 : 2000)) {
	...
	private repairActions(worker: Bot): boolean {
	...
	private buildActions(worker: Bot): boolean {
	...
	private dismantleActions(worker: Bot): boolean {
	...
	private pavingActions(worker: Bot): boolean {
		const roomToRepave = this.brain.roadLogistics.workerShouldRepave(worker)!;
		this.brain.roadLogistics.registerWorkerAssignment(worker, roomToRepave);
		// Build a paving manifest
		const pavingManifest = this.brain.roadLogistics.buildPavingManifest(worker, roomToRepave);
		if (pavingManifest) {
	...
	private fortifyActions(worker: Bot, fortifyStructures = this.fortifyBarriers): boolean {
		...
		if (highestBarrierHits > WorkerManager.settings.hitTolerance) {
			...
			lowBarriers = _.filter(fortifyStructures, structure => structure.hits <= lowestBarrierHits +
                WorkerManager.settings.hitTolerance);
		} else {
	...
	private nukeFortifyActions(worker: Bot, fortifyStructures = this.nukeDefenseRamparts): boolean {
	...
	private upgradeActions(worker: Bot): boolean {
		// Sign controller if needed
		if ((!this.brain.controller.signedByMe && !this.brain.controller.signedByScreeps)) {
			worker.task = Tasks.signController(this.brain.controller);
			return true;
		}
	...
	private handleWorker(worker: Bot) {
		if (worker.carry.energy > 0) {
			...
			const downgradeLevel = CONTROLLER_DOWNGRADE[this.brain.controller.level] *
								   (this.brain.controller.level < 4 ? .3 : .7);
			if ((!this.brain.controller.upgradeBlocked || this.brain.controller.upgradeBlocked < 30)
				&& (this.brain.controller.ticksToDowngrade <= downgradeLevel
					|| this.brain.controller.progress > this.brain.controller.progressTotal)) {
				if (this.upgradeActions(worker)) return;
			}
			// Repair damaged non-road non-barrier structures
			if (this.repairStructures.length > 0 && this.brain.defcon == DEFCON.safe) {
			...
			// if (this.nukeDefenseRamparts.length > 0 && !this.brain.state.isRebuilding) {
			// 	if (this.nukeFortifyActions(worker, this.nukeDefenseRamparts)) return;
			// }
			// // Build and maintain roads
			// if (this.brain.roadLogistics.workerShouldRepave(worker) && this.brain.defcon == DEFCON.safe) {
			// 	if (this.pavingActions(worker)) return;
			// }
			// Dismantle marked structures
			if (this.dismantleStructures.length > 0 && this.brain.defcon == DEFCON.safe) {
			...
			if ((this.brain.level < 8 || this.brain.upgradeSite.manager.upgraders.length == 0)
				&& this.brain.defcon == DEFCON.safe) {
				if (this.upgradeActions(worker)) return;
			}
		} else {
			// Acquire more energy
			const workerWithdrawLimit = this.brain.stage == brainStage.Infant ? 750 : 100;

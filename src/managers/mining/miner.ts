...
import { Manager, ManagerMemory } from "managers/Manager";
...
export const DoubleMinerSetupCost = bodyCost(Setups.worker.miner.double.generateBody(Infinity));
...
interface MiningManagerMemory extends ManagerMemory {
...
export class MiningManager extends Manager {
	...
	constructor(directive: DirectiveHarvest, priority: number) {
		...
		} else if (this.brain.level >= DirectiveOutpost.settings.canSpawnReserversAtRCL) {
		...
		if (Cartographer.roomType(this.pos.roomName) == ROOMTYPE_SOURCEKEEPER) {
			this.mode = 'SK';
			this.setup = Setups.worker.miner.sourceKeeper;
		} else if (this.brain.room.energyCapacityAvailable < StandardMinerSetupCost) {
			this.mode = 'early';
		...
		// }
			...
			this.mode = 'link';
			if (this.brain.assets.energy >= 100000) {
				this.setup = Setups.worker.miner.linkOptimized;
			} else {
				this.setup = Setups.worker.miner.default;
			}
		} else {
			this.mode = 'standard';
			// this.setup = Game.cpu.bucket < 9500 ? Setups.worker.miner.standardCPU : Setups.worker.miner.standard;
			this.setup = Setups.worker.miner.standard;
	...
	private getDismantlePositions(): RoomPosition[] {
		...
		if (this.room) {
			...
			log.error(`MiningManager.getDismantleStructures() called with no vision in room ${this.pos.roomName}!`);
		}
	...
	private addRemoveContainer(): void {
		...
		if (this.container && this.link) {
			// safety checks
			if (this.brain.spawner && this.container.pos.getRangeTo(this.brain.spawner) > 2 &&
				this.container.pos.getRangeTo(this.brain.upgradeSite) > 3) {
	...
	private earlyMiningActions(miner: Bot) {
		...
		if (this.container) {
			if (this.container.hits < this.container.hitsMax
				...
				if (miner.carry.getFreeCapacity()) {
	 ...
	 */
	private skMiningActions(miner: Bot) {
		// Sleep until your source regens
		if (this.isSleeping(miner)) return;
	 ...
	 */
	private doubleMiningActions(miner: Bot): any {
	 ...
	 * Actions for handling mining at RCL high enough to spawn ideal miner body to saturate source
	 */
	private dismantleActions(miner: Bot): any {
	...
	private isSleeping(miner: Bot): boolean {
	 ...
	 */
	private harvestOrSleep(miner: Bot, source: Source, allowSuicide = true): void {

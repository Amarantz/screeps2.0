import { printRoomName, rightArrow, leftArrow, alignedNewline, maxBy, minBy, bullet } from "utils/utils";
...
export class TraderJoe implements ITradeNetwork {
	...
	refresh() {
		this.memory = Mem.wrap(Memory.BigBrain, 'trader', getDefaultTraderMemory);
	...
	private computeEffectiveEnergyPrices(): void {
		...
		for (const colony of _.sample(getAllBrains(), 5)) {
	...
	private cleanOrders() {
		const ordersToClean = _.filter(Game.market.orders, order => {
			...
			if (order.roomName && !BigBrain.brains[order.roomName]) {
				return true;
            }
            return false;
		});

...
export class RoadLogistics {
	...
	constructor(brain: Brain) {
		this.brain = brain;
		this.ref = this.brain.name + ':roadLogistics';
	 ...
	 */
	private workerShouldRepaveRoom(worker: Bot, room: Room): boolean {
	 ...
	 */
	workerShouldRepave(worker: Bot): Room | undefined {
		...
		for (const room of this.brain.rooms) {
			if (this.brain.isRoomActive(room.name) && room.isSafe && this.workerShouldRepaveRoom(worker, room)) {
				return room;
			}
        }
        return;
		...
		return $.structures(this, 'criticalRoads:' + room.name, () => {
			const criticalRoads = _.filter(room.roads,
										   road => road.hits < road.hitsMax * RoadLogistics.settings.criticalThreshold
												   && this.brain.roomPlanner.roadShouldBeHere(road.pos));
			return _.sortBy(criticalRoads, road => road.pos.getMultiRoomRangeTo(this.brain.pos));
		}, ROAD_CACHE_TIMEOUT);
	}
		...
		return $.structures(this, 'repairableRoads:' + room.name, () => {
			const repairRoads = _.filter(room.roads,
										 road => road.hits < road.hitsMax * RoadLogistics.settings.repairThreshold
												 && this.brain.roomPlanner.roadShouldBeHere(road.pos));
			return _.sortBy(repairRoads, road => road.pos.getMultiRoomRangeTo(this.brain.pos));
		}, ROAD_CACHE_TIMEOUT);
	}
	...
	unbuiltRoads(room: Room): RoomPosition[] {
		...
		return $.list(this, 'repairableRoads:' + room.name, () => {
            const roadPositions = this.brain.roomPlanner.roadPlanner.getRoadPositions(room.name);
            //@ts-ignore
            const unbuiltPositions = _.filter(roadPositions, pos => !pos.lookForStructure(STRUCTURE_ROAD));
            //@ts-ignore
			return _.sortBy(unbuiltPositions, pos => pos.getMultiRoomRangeTo(this.brain.pos));
		}, ROAD_CACHE_TIMEOUT);
	}
	 ...
	 * on the same tick
	 */
	registerWorkerAssignment(worker: Bot, room: Room): void {
	...
	init(): void {
		const workers = this.brain.managers.work.workers;
	...
	buildPavingManifest(worker: Bot, room: Room): Task | null {

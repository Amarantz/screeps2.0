...
import { unpackCoordListAsPosList, packCoord, packCoordList } from "utils/packrat";
import { RoomPlanner } from "./RoomPlanner";
import { log } from "console/log";
import { Pathing, MatrixTypes } from "movement/Pathing";
import { posFromReadableName, onPublicServer, getCacheExpiration } from "utils/utils";
...
export class RoadPlanner {
	...
	constructor(roomPlanner: RoomPlanner) {
		...
		this.memory = Mem.wrap(this.brain.memory, 'roadPlanner', getDefaultRoadPlannerMemory);
		this.costMatrices = {};
		this.roadPositions = [];
	}
	...
	private recomputeRoadCoverages(storagePos: RoomPosition, ignoreInactiveRooms = true) {
		...
		for (const destination of this.brain.destinations) {
		...
		for (const destName in this.memory.roadCoverages) {
			...
			if (ignoreInactiveRooms && !this.brain.isRoomActive(destPos.roomName)) {
				continue;
			}
	...
	private buildRoadNetwork(storagePos: RoomPosition, obstacles: RoomPosition[]): void {
		this.costMatrices = {};
		this.roadPositions = [];
		const destinations: { pos: RoomPosition, order: number }[] = _.sortBy(this.brain.destinations, destination => destination.order);
		// Connect commandCenter to each destination in brain
	...
	private generateRoadPath(origin: RoomPosition, destination: RoomPosition,
		...
		const callback = (roomName: string): CostMatrix | boolean => {
			if (!this.brain.roomNames.includes(roomName)) { // only route through brain rooms
				return false;
			}
		...
		if (ret.incomplete) {
			log.warning(`Roadplanner for ${this.brain.print}: could not plan road path!`);
			return;
		}
	...
	finalize(): void {
		...
		if (_.keys(this.roomPlanner.map).length > 0) { // use active map
			...
			if (this.roomPlanner.memory.bunkerData && this.roomPlanner.memory.bunkerData.anchor) {
				...
				log.error(`RoadPlanner@${this.brain.room.print}: could not get road positions from room planner!`);
				roomPlannerRoads = [];
			}
		}
	...
	private buildMissing(): void {
		...
		const origin = (this.brain.storage || this.brain.spawner || this.brain).pos;
				...
				if (count > 0) {
					...
					if (ret != OK) {
						if (ret == ERR_NOT_OWNER) {
							if (Game.time % 50 == 0) {
								log.warning(`${this.brain.print}: couldn't create road site at ${pos.print}; room ` +
							...
							// murder your CPU.
							log.warning(`${this.brain.print}: couldn't create road site at ${pos.print}, too many ` +
										`construction sites!`);
							break;
						} else {
							log.warning(`${this.brain.print}: couldn't create road site at ${pos.print} (${ret})`);
						}
					} else {
						count--;
					}
	...
	private cleanRoadCoverage() {
        //@ts-ignore
		const brainDestinations = this.brain.destinations.map(dest => `${dest.pos.roomName}:${dest.pos.x}:${dest.pos.y}`);
		console.log(`Brain ${this.brain.print} has destinations of ${JSON.stringify(brainDestinations)}`);

		for (const roadCoverageKey of Object.keys(this.memory.roadCoverages)) {
			// console.log(`Brain ${this.brain.name} Road coverage of ${roadCoverageKey}`);
			if (brainDestinations.includes(roadCoverageKey)) {
				// console.log(`Brain has destination of ${roadCoverageKey}`);
			} else {
				console.log(`Brain does not have destination of ${roadCoverageKey}, deleting.`);
	...
	run(): void {
		if (this.roomPlanner.active) {
			...
			if (Game.time % RoadPlanner.settings.recalculateRoadNetworkInterval == this.brain.id) {
				if (this.roomPlanner.storagePos) {
			...
			if (Game.time % getAllBrains().length == this.brain.id && this.roomPlanner.storagePos) {
				this.recomputeRoadCoverages(this.roomPlanner.storagePos);
			}
			// Build missing roads
			if (this.brain.level >= RoadPlanner.settings.buildRoadsAtRCL && this.roomPlanner.shouldRecheck(3)) {
				this.buildMissing();
			}
		}
	}

...
import { BUNKER_RADIUS, bunkerLayout, allBunkerCoords, bunkerCoordLookup } from './layout/bunker';
const MAX_SAMPLE = 10;
...
export class BasePlanner {
	...
	static getBunkerLocation(room: Room, visualize = true): RoomPosition | undefined {
		const brain = BigBrain.brains[room.name] as Brain;
		if (brain && brain.bunker && brain.bunker.anchor) {
			return brain.bunker.anchor;
		}

...
import { Mem, Autonomy, getAutonomyLevel } from 'memory/memory';
import { DemolishStructurePriorities, BuildPriorities } from 'priorities/priorities_structures';
import { onPublicServer, bullet, hasMinerals, maxBy } from 'utils/utils';
import { BarrierPlanner } from './BarrierPlanner';
import { log } from 'console/log';
import { RoadPlanner } from './RoadPlanner';
import { bunkerLayout } from './layout/bunker';
...
export class RoomPlanner {
	brain: Brain;							// The brain this is for
	...
	placements: { 							// Used for generating the plan
		spawner: RoomPosition | undefined;
	...
	static settings = {
		...
		maxSitesPerBrain : onPublicServer() ? 10 : 25,
		maxDismantleCount : 5,
	};

	constructor(brain: Brain) {
		this.brain = brain;
		this.memory = Mem.wrap(this.brain.memory, 'roomPlanner', getDefaultRoomPlannerMemory);
	...
	refresh(): void {
		...
		if (this.active && Game.time % 25 == 0) {
			log.alert(`RoomPlanner for ${this.brain.room.print} is still active! Close to save CPU.`);
		}
	}
	 ...
	 */
	private recallMap(level = this.brain.controller.level): void {
	 ...
	 */
	get spawnerPos(): RoomPosition | undefined {
		if (this.placements.spawner) {
			return this.placements.spawner;
		}
	...
	get bunkerPos(): RoomPosition | undefined {
		...
		if (this.memory.bunkerData && this.memory.bunkerData.anchor) {
			return new RoomPosition(this.memory.bunkerData.anchor.x, this.memory.bunkerData.anchor.y, this.brain.name);
		}
		return
	}
	...
	private reactivate(): void {
		...
		const msg = [
			`Room planner activated for ${this.brain.name}. Reinstantiating flags from previous session on next tick.`,
			'Place brain components with room planner flags:',
			bullet + 'Place bunker (recommended)  white/red',
			bullet + 'Place spawner:             white/green',
	...
	addComponent(componentName: 'spawner' | 'commandCenter' | 'bunker', pos: RoomPosition, rotation = 0): void {
	...
	private generatePlan(level = 8): RoomPlan {
		...
		for (const name in this.placements) {
			...
			if (layout) {
				...
				const pos = this.placements[<'spawner' | 'commandCenter' | 'bunker'>name];
				if (!pos) continue;
				...
				plan[name] = {
					map     : componentMap,
					pos     : new RoomPosition(anchor.x, anchor.y, this.brain.name),
					rotation: 0,
				};
			}
		}
		...
		if (layout) {
			for (const buildingName in layout.buildings) {
				map[buildingName] = _.map(layout.buildings[buildingName].pos,
										  pos => new RoomPosition(pos.x, pos.y, this.brain.name));
			}
		}
		return map;
	}
	...
	getStructureMapForBunkerAt(anchor: { x: number, y: number }, level = 8): StructureMap {
		...
		return _.mapValues(structureLayout, coordArr =>
			_.map(coordArr, coord => new RoomPosition(coord.x + dx, coord.y + dy, this.brain.name)));
	}
	...
	getBunkerStructurePlacement(structureType: string, anchor: { x: number, y: number },
		...
		return _.map(bunkerLayout[level]!.buildings[structureType].pos,
					 coord => new RoomPosition(coord.x + dx, coord.y + dy, this.brain.name));
	}
	...
	getObstacles(): RoomPosition[] {
		...
		obstacles.concat(_.map(this.brain.sources, source => source.pos));
		obstacles.concat(_.map(this.brain.extractors, extr => extr.pos));
	...
	private findCollision(ignoreRoads = false): RoomPosition | undefined {
		const terrain = Game.map.getRoomTerrain(this.brain.room.name);
		for (const structureType in this.map) {
	...
	finalize(ignoreRoads = false): void {
		...
		const layoutIsValid: boolean = (!!this.placements.commandCenter && !!this.placements.spawner)
		...
		if (layoutIsValid) { // Write everything to memory
			...
			const flagsToWrite = _.filter(this.brain.flags, flag => flag.color == COLOR_WHITE);
			...
			if (this.brain.level == 1) { // clear out room if setting in for first time
				this.demolishMisplacedStructures(true, true);
				// Demolish all barriers that aren't yours
				for (const barrier of this.brain.room.barriers) {
			...
			log.warning('Not a valid room layout! Must have both spawner and commandCenter placements ' +
						'or bunker placement.');
		}
	}
	...
	structureShouldBeHere(structureType: StructureConstant, pos: RoomPosition,
						  level = this.brain.controller.level): boolean {
		if (structureType == STRUCTURE_ROAD) {
			...
			if (structureType == STRUCTURE_CONTAINER || structureType == STRUCTURE_LINK) {
				const thingsBuildingLinksAndContainers = _.map([...this.brain.room.sources,
																this.brain.room.mineral!,
																this.brain.controller], thing => thing.pos);
				const maxRange = 4;
	...
	private demolishHostileStructures(destroyStorageUnits = false) {
		_.forEach(this.brain.room.walls, wall => wall.destroy()); // overmind never uses walls
		for (const structure of _.filter(this.brain.room.hostileStructures)) {
	...
	private removeMisplacedConstructionSites() {
		for (const site of this.brain.room.find(FIND_CONSTRUCTION_SITES)) {
			if (site.owner.username != SETTINGS.MY_USERNAME) {
				site.remove();
	...
	private demolishMisplacedStructures(skipRamparts = true, destroyAllStructureTypes = false): void {
		...
		if (getAllBrains().length <= 1 && !this.brain.storage) {
		...
		if (this.brain.terminal) {
			if (this.brain.storage && !this.structureShouldBeHere(STRUCTURE_STORAGE, this.brain.storage.pos)
				|| !this.structureShouldBeHere(STRUCTURE_TERMINAL, this.brain.terminal.pos)) {
				// DirectiveTerminalRebuildState.createIfNotPresent(this.brain.terminal.pos, 'pos');
			}
		}

		// Max buildings that can be placed each tick
		const count = RoomPlanner.settings.maxSitesPerBrain - this.brain.constructionSites.length;

		// Recall the appropriate map
		this.recallMap();
		if (!this.map || this.map == {}) { // in case a map hasn't been generated yet
			log.info(this.brain.name + ' does not have a room plan yet! Unable to demolish errant structures.');
		}

		// Destroy extractor if needed
		if (this.brain.room.extractor && !this.brain.room.extractor.my) {
			this.brain.room.extractor.destroy();
		}
		...
		for (const priority of DemolishStructurePriorities) {
			...
			// if (structureType == STRUCTURE_RAMPART && this.brain.layout == 'bunker') {
			// 	let bunkerBarriers = _.filter(this.brain.room.barriers, b => insideBunkerBounds(b.pos, this.brain));
			...
			let removeCount = 0;
			let structures: Structure[] = _.filter(this.brain.room.find(FIND_STRUCTURES),
			...
			for (const structure of structures) {
				...
				if (!this.structureShouldBeHere(structureType, structure.pos) ||
					...
					if (this.brain.level >= 6 && structureType == STRUCTURE_TERMINAL) {
						if (!this.brain.storage) {
							log.info(`${this.brain.name}: waiting until storage is built to remove terminal`);
							return;
						} else if (this.brain.terminal &&
									//@ts-ignore
								   _.sum(this.brain.terminal.store) - this.brain.terminal.energy > 1000) {
							log.info(`${this.brain.name}: waiting on resources to evacuate before removing terminal`);
							return;
						} else if (this.brain.storage &&
								   this.structureShouldBeHere(STRUCTURE_STORAGE, this.brain.storage.pos) &&
								   this.brain.storage.energy
								   < Energetics.settings.storage.energy.destroyTerminalThreshold) {
							log.info(`${this.brain.name}: waiting to move energy to storage before removing terminal`);
							return;
						}
					}

					// Only remove a maximum number of structures at a time
					const amountMissing = CONTROLLER_STRUCTURES[structureType][this.brain.level] - structures.length
										  + removeCount;
					if (amountMissing < maxRemoved) {
						if (structureType == STRUCTURE_SPAWN && this.brain.spawns.length == 1) {
							const spawnCost = 15000;
							if (this.brain.assets[RESOURCE_ENERGY] < spawnCost) {
								log.warning(`${this.brain.print}: Unsafe to destroy misplaced spawn: ` +
											`${this.brain.assets[RESOURCE_ENERGY]}/${spawnCost} energy available`);
								if (!destroyAllStructureTypes) {
									return;
								}
							}
							const workTicksNeeded = 15000 / BUILD_POWER;
							const workTicksAvailable = _.sum(this.brain.managers.work.workers, worker =>
						...
						if (result != OK) {
							log.warning(`${this.brain.name}: couldn't destroy structure of type ` +
										`"${structureType}" at ${structure.pos.print}. Result: ${result}`);
						} else {
							log.info(`${this.brain.name}: destroyed ${structureType} at ${structure.pos.print}`);
						}
						removeCount++;
	...
	private buildMissingStructures(): void {
		...
		this.recallMap();
		if (!this.map || this.map == {}) { // in case a map hasn't been generated yet
			log.info(this.brain.name + ' does not have a room plan yet! Unable to build missing structures.');
		}
		// Build missing structures from room plan
						...
						if (result != OK) {
							...
							for (const structure of structures) {
								...
								if (!this.structureShouldBeHere(structure.structureType, pos)
									...
									if (result == OK) {
										log.info(`${this.brain.name}: destroyed ${structure.structureType} at` +
												 ` ${structure.pos.print}`);
										this.memory.recheckStructuresAt = Game.time +
																		  RoomPlanner.settings.recheckAfter;
									} else {
										log.warning(`${this.brain.name}: couldn't destroy ${structure.structureType}` +
													` at ${structure.pos.print}! Result: ${result}`);
									}
								}
							}
							log.warning(`${this.brain.name}: couldn't create construction site of type ` +
		...
		const mineral = this.brain.room.find(FIND_MINERALS)[0];
		if (mineral) {
	...
	private calculateLinkPos(anchor: RoomPosition): RoomPosition | undefined {
		if (anchor.isEqualTo(this.brain.controller.pos)) {
	...
	private calculateUpgradeSiteLinkPos(): RoomPosition | undefined {
		...
		if (originPos && this.brain.upgradeSite.batteryPos) {
			// Build link at last location on path from origin to battery
			const path = Pathing.findShortestPath(this.brain.upgradeSite.batteryPos, originPos).path;
			return path[0];
		}
		return;
	}
	...
	private nextNeededLinkAnchor(): RoomPosition | undefined {
		const linksEtAl = _.map((<(StructureLink | ConstructionSite)[]>[])
									.concat(this.brain.links, _.filter(this.brain.constructionSites,
																		site => site.structureType == STRUCTURE_LINK)),
								s => s.pos);
		// UpgradeSite goes first
		const upgradeLink = this.brain.controller.pos.findClosestByLimitedRange(linksEtAl, 3);
		if (!upgradeLink) return this.brain.controller.pos;
		// MiningSites by decreasing distance
		const origin = (this.brain.storage || this.brain.terminal || _.first(this.brain.spawns) || this.brain).pos;
		const farthestSources = _.sortBy(this.brain.room.sources,
	...
	private buildNeededLinks() {
		const numLinks = this.brain.links.length +
						 _.filter(this.brain.constructionSites, site => site.structureType == STRUCTURE_LINK).length;
		const numLinksAllowed = CONTROLLER_STRUCTURES.link[this.brain.level];
		if (numLinksAllowed > numLinks &&
			(this.brain.bunker || (this.brain.spawner && this.brain.spawner.link)) &&
			this.brain.commandCenter && this.brain.commandCenter.link) {
	...
	init(): void {
		if (this.active) {
			BigBrain.CEO.notifier.alert(`Room planner active!`, this.brain.room.name);
		}
		if (this.active && getAutonomyLevel() == Autonomy.Automatic && !this.memory.bunkerData) {
			let bunkerAnchor: RoomPosition;
			if (this.brain.spawns.length > 0) { // in case of very first spawn
				const lowerRightSpawn = maxBy(this.brain.spawns, s => 50 * s.pos.y + s.pos.x)!;
				const spawnPos = lowerRightSpawn.pos;
				bunkerAnchor = new RoomPosition(spawnPos.x - 4, spawnPos.y, spawnPos.roomName);
			} else {
				const expansionData = RoomIntel.getExpansionData(this.brain.room.name);
				if (expansionData) {
	...
	shouldRecheck(offset = 0): boolean {
		if (Game.time == (this.memory.recheckStructuresAt || Infinity) + offset) {
			...
			const checkFreq = RoomPlanner.settings.siteCheckFrequency * this.brain.level;
			return Game.time % checkFreq == (2 * this.brain.id + offset) % checkFreq;
		}
	}

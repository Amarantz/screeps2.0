...
import { isCombatBot } from "declarations/typeGuards";
...
export class GoalFinder {
	...
	static skirmishGoals(bot: CombatBot): { approach: PathFinderGoal[], avoid: PathFinderGoal[] } {
		...
		const myAttack = CombatIntel.getAttackDamage(bot);
		const myRangedAttack = CombatIntel.getRangedAttackDamage(bot);
		const myHealing = CombatIntel.getHealAmount(bot);

		// If you're purely a healer, ignore combat goals
		if (myHealing > 0 && myAttack == 0 && myRangedAttack == 0) {
			return this.healingGoals(bot);
		}

		const preferCloseCombat = myAttack > 0;
		const myRating = CombatIntel.rating(bot);
		const nearbyRating = _.sum(bot.pos.findInRange(room.creeps, 6), c => CombatIntel.rating(c));
		const braveMode = bot.hits * (nearbyRating / myRating) * .5 > bot.hitsMax;
		...
		for (const hostile of room.hostiles) {
			...
			analysis[hostile.id] = {
				...
				advantage    : healing == 0 || attack + rangedAttack == 0 ||
							   myAttack + myRangedAttack + myHealing / CombatIntel.minimumDamageTakenMultiplier(bot.creep)
							   > attack + rangedAttack + healing / CombatIntel.minimumDamageTakenMultiplier(hostile),
				isRetreating : CombatIntel.isRetreating(hostile, RoomIntel.getPreviousPos(bot.creep)),
				isApproaching: CombatIntel.isApproaching(hostile, RoomIntel.getPreviousPos(bot.creep)),
			};
		}
		...
		for (const target of approachTargets) {
			...
			if (data && (data.advantage || braveMode)) {
				...
				if (!preferCloseCombat && (data.attack > 0 || data.rangedAttack > myRangedAttack)) {
					range = bot.pos.getRangeTo(target) == 3 && data.isRetreating ? 2 : 3;
		...
		if (DEBUG) {
			log.debug(`Report for ${bot.name}:`, JSON.stringify(analysis));
			log.debug(`Approach for ${bot.name}:`, JSON.stringify(approach));
			log.debug(`Avoid for ${bot.name}:`, JSON.stringify(avoid));
		}
	...
	static retreatGoals(creep: CombatBot): { approach: PathFinderGoal[], avoid: PathFinderGoal[] } {
		const approach: PathFinderGoal[] = [];
		const avoid: PathFinderGoal[] = [];

		const isHealer = CombatIntel.isHealer(creep);
		for (const friendly of creep.room.creeps) {
			if (CombatIntel.getHealPotential(friendly) > 0 || (isHealer && isCombatBot(creep))) {
	...
	static healingGoals(healer: CombatBot): { approach: PathFinderGoal[], avoid: PathFinderGoal[] } {
		const approach: PathFinderGoal[] = [];
		const avoid: PathFinderGoal[] = [];
	...
	static structureGoals(creep: CombatBot): { approach: PathFinderGoal[], avoid: PathFinderGoal[] } {
		const approach: PathFinderGoal[] = [];

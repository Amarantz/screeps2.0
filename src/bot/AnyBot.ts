...
import {isAnyBot, isPowerCreep} from '../declarations/typeGuards';
...
export function getManager(creep: AnyBot | AnyCreep): Manager | null {
	if (creep.memory[MEM.MANAGER]) {
		return BigBrain.managers[creep.memory[MEM.MANAGER]!] || null;
	} else {
		return null;
	}
}

export function setManager(creep: AnyBot | AnyCreep, newManager: Manager | null) {
	// Remove cache references to old assignments
	const roleName = creep.memory.role;
	const ref = creep.memory[MEM.MANAGER];
	const oldManager: Manager | null = ref ? BigBrain.managers[ref] : null;
	if (ref && BigBrain.cache.managers[ref] && BigBrain.cache.managers[ref][roleName]) {
		_.remove(BigBrain.cache.managers[ref][roleName], name => name == creep.name);
	}
	if (newManager) {
		// Change to the new overlord's brain
		creep.memory[MEM.BRAIN] = newManager.brain.name;
		// Change assignments in memory
		creep.memory[MEM.MANAGER] = newManager.ref;
		// Update the cache references
		if (!BigBrain.cache.managers[newManager.ref]) {
			BigBrain.cache.managers[newManager.ref] = {};
		}
		if (!BigBrain.cache.managers[newManager.ref][roleName]) {
			BigBrain.cache.managers[newManager.ref][roleName] = [];
		}
		BigBrain.cache.managers[newManager.ref][roleName].push(creep.name);
	} else {
		creep.memory[MEM.MANAGER] = null;
	}
	if (oldManager) oldManager.recalculateCreeps();
	if (newManager) newManager.recalculateCreeps();
}

export function normalizeAnyBot(creep: AnyBot | AnyCreep): AnyBot | AnyCreep {
	return BigBrain.bots[creep.name] || BigBrain.powerBots[creep.name] || creep;
}
...
export abstract class AnyBot {
	...
	transfer(target: AnyCreep | AnyBot | Structure, resourceType: ResourceConstant = RESOURCE_ENERGY, amount?: number) {
	...
	transferAll(target: AnyCreep | AnyBot | Structure, amount?: number) {
	...
	goTransfer(target: Creep | AnyBot | Structure, resourceType: ResourceConstant = RESOURCE_ENERGY,
			   amount?: number): void {
	...
	get overlord(): Manager | null {
		return getManager(this);
	}

	set overlord(newManager: Manager | null) {
		setManager(this, newManager);
	}

	// Brain association ----------------------------------------------------------------------------------------------

	/**
	 * Brain that the creep belongs to.
	 */
	get brain(): Brain | null {
		if (this.memory[MEM.BRAIN] != null) {
			return BigBrain.brains[this.memory[MEM.BRAIN] as string];
		} else {
			return null;
		}
	}

	set brain(newBrain: Brain | null) {
		if (newBrain != null) {
			this.memory[MEM.BRAIN] = newBrain.name;
		} else {
			this.memory[MEM.BRAIN] = null;
		}
	}

	/**
	 * If the creep is in a brain room or outpost
	 */
	get inBrainRoom(): boolean {
		return BigBrain.brainsMaps[this.room.name] == this.memory[MEM.BRAIN];
	}
	...
	avoidDanger(opts: FleeOptions = {}): boolean {
		...
		if (!this.room.isSafe || this.hits < this.hitsMax) {
			...
			if (this.brain && Game.map.getRoomLinearDistance(this.room.name, this.brain.name) <= maxLinearRange) {
				fallback = this.brain.name;
			}
			// But this could happen if the creep was working remotely through a portal
			else {
				const nearbyBrains = _.filter(getAllBrains(), brain =>
					Game.map.getRoomLinearDistance(this.room.name, brain.name) <= maxLinearRange);
				const closestBrain = minBy(nearbyBrains, brain => {
					const route = Pathing.findRoute(this.room.name, brain.room.name);

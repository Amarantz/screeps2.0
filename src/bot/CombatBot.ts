...
import { Movement, CombatMoveOptions } from "movement/Movement";
import { CombatIntel } from "intel/CombatIntel";
import { GoalFinder } from "targeting/GoalFinder";

interface CombatBotMemory extends CreepMemory {
	recovering: boolean;
	lastInDanger: number;
	partner?: string;
	swarm?: string;
...
/**
 * CombatBot is an extension of the Bot class which contains additional combat-related methods
 */
export class CombatBot extends Bot {
	...
	findPartner(partners: CombatBot[], tickDifference = DEFAULT_PARTNER_TICK_DIFFERENCE): CombatBot | undefined {
	...
	findSwarm(partners: CombatBot[], maxByRole: { [role: string]: number },
	...
	autoBunkerCombat(roomName: string, verbose = false) {
		...
		// TODO check if right brain, also yes brain check is in there to stop red squigglies
		// const siegingCreeps = this.room.hostiles.filter(creep =>
		// 	_.any(creep.pos.neighbors, pos => this.brain && insideBunkerBounds(pos, this.brain)));

		const target = CombatTargeting.findTarget(this, this.brain ? this.room.playerHostiles
																		  .filter(creep => creep.pos.getRangeTo(this.brain!.pos) <= 9) : this.room.dangerousHostiles);

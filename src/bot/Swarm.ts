...
import { Movement, CombatMoveOptions, SwarmMoveOptions } from "movement/Movement";
import { log } from "console/log";
import { CombatTargeting } from "targeting/CombatTarging";
import { DEFAULT_SWARM_TICK_DIFFERENCE, CombatBot } from "./CombatBot";
import { normalizePos } from "movement/helpers";
import { hasPos } from "declarations/typeGuards";
import { rotatedMatrix, getCacheExpiration } from "utils/utils";
import { CombatIntel } from "intel/CombatIntel";
import { Mem } from "memory/memory";
import { CombatManager } from "managers/CombatManager";

export interface ProtoSwarm {
	creeps: Creep[] | CombatBot[];
}
...
interface SwarmManager extends CombatManager {
	memory: any;
}
...
export class Swarm implements ProtoSwarm {
	...
	memory: SwarmMemory;
	ref: string;
	creeps: CombatBot[];							// All creeps involved in the swarm
	uniformCreepType: boolean;						// Whether the swarm is composed of all one type of creep
	formation: (CombatBot | undefined)[][]; 		// Relative ordering of the creeps accounting for orientation
	staticFormation: (CombatBot | undefined)[][];	// Relative ordering of the creeps assuming a TOP orientation
	...
	constructor(manager: SwarmManager, ref: string, creeps: CombatBot[], width = 2, height = 2) {
		this.manager = manager;
		this.ref = ref;
		this.memory = Mem.wrap(manager.memory, `swarm:${ref}`, getDefaultSwarmMemory);
		// Build the static formation by putting attackers at the front and healers at the rear
		const paddedCreeps: (CombatBot | undefined)[] = _.clone(creeps);
	...
	private getCreepScores(creeps: (CombatBot | undefined)[]): { [name: string]: number } {
	...
	private printFormation(formation: (CombatBot | undefined)[][]): string {
	...
	static cleanMemory(manager: { swarms: { [ref: string]: Swarm }, memory: any }) {
		for (const ref in manager.swarms) {
			// TODO
		}
	}
